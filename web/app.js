...
function startQuick(target){if(quickEdit)throw Error('Save or cancel the current cell first.');const r=record(target.dataset.id),u=r.instances[Number(target.dataset.index)],key=target.dataset.field;
 quickEdit={record_id:r.id,unit_id:u.id,rev:r.rev,key};const td=target.closest('td,.unit-field');td.innerHTML=`<div class="quick-control">${unitControl(key,u[key],'quick-value')}<button data-action="quick-save">Save</button><button data-action="quick-cancel">Cancel</button></div>`;($('quick-value').closest?.('.catalog-combo')?.querySelector('[data-combo-query]')||$('quick-value')).focus();}
...
function unitCell(r,u,i,key){
 ...
 if(key==='model')return `<strong>${esc(r.model)}</strong><small>${esc(forParts(u)?'For parts':enumLabel(u.condition))}</small>`;
...
function expandedRow(r,visible){return r.kind==='phone'?`${shownUnits(r).map(u=>{const i=r.instances.indexOf(u);return `<tr class="unit-table-row">${visible.map(k=>`<td data-column="${k}" class="wrap">${unitCell(r,u,i,k)}</td>`).join('')}</tr><tr class="unit-extra-row"><td colspan="${visible.length}">${unitExtra(r,u,i)}</td></tr>`;}).join('')}<tr class="row-expanded"><td colspan="${visible.length}">${modelActions(r)}</td></tr>`:'';}
