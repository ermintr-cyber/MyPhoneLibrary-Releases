...
$('record-form').addEventListener('submit',async e=>{e.preventDefault();if(uploadCount){toast('Photos are still uploading.');return;}if(!validateCombos($('editor-content')))return;readDraft();const duplicates=imeiWarnings()||[];if(duplicates.length&&!confirm('Duplicate IMEI detected. Review the warning above. Save anyway?'))return;$('record-save').disabled=true;try{const r=await api('/api/record',{record:draft,rev:draft.rev});await refresh();if(mode==='add')phoneDraft=null;dirty=false;$('editor').close();draft=null;expanded.add(r.id);render();toast('Phone / record saved.');}catch(error){$('editor-error').textContent=error.message;}finally{$('record-save').disabled=false;}});
