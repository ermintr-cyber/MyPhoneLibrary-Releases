...
function comparisonRows(entries){
 ...
 const customKeys=[...new Set(entries.flatMap(({r})=>Object.keys(r.custom||{})))];
 for(const key of customKeys){const f=(db.settings.custom_fields||[]).find(f=>f.id===key);fields.push(['Custom: '+(f?.label||key),(u,r)=>r.custom?.[key]]);}
 const specKeys=[...new Set(entries.flatMap(({r})=>Object.keys(r.specs||{})))];
 for(const key of specKeys)fields.push(['Specification: '+key,(u,r)=>r.specs?.[key]]);
 return fields.map(([label,get])=>({label,values:entries.map(({u,r})=>comparisonText(get(u,r)))})).filter(row=>row.values.some(v=>v!==''));
}
function compareUnits(){const entries=db.records.flatMap(r=>(r.instances||[]).filter(u=>selectedUnits.has(u.id)).map(u=>({r,u})));if(entries.length<2||entries.length>20)throw Error('Select 2 to 20 phones to compare. Different models are welcome.');panel('Compare — '+entries.length+' phones',`<p>Differences are highlighted. Fields empty on every selected phone are hidden.</p><div class="comparison-scroll"><table class="comparison-table"><thead><tr><th>Property</th>${entries.map(({r,u})=>`<th>${unitImage(r,u)}<strong>${esc(name(r))}</strong><strong>#${esc(u.inv)}</strong><button data-action="edit-unit" data-id="${r.id}" data-index="${r.instances.indexOf(u)}">Edit</button></th>`).join('')}</tr></thead><tbody>${comparisonRows(entries).map(({label,values})=>`<tr class="${new Set(values).size>1?'comparison-different':''}"><th>${esc(label)}</th>${values.map(v=>`<td>${esc(v)||'—'}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`,{kind:'compare'});}
