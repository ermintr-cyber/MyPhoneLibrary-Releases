...
const opts={state:['Netestiran','Ispravan','Djelimično ispravan','Neispravan'],condition:['U kolekciji','Posuđen','Prodan','Poklonjen','Rastavljen','Rashodovan'],purpose:['Kolekcija','Za popravak','Donor','Za prodaju','Za razmjenu'],edition:['Standard','Music Edition','Limited Edition'],currency:['CHF','EUR','BAM','USD'],originality:['Nepoznato','Original','Zamjenski','Miješano'],lock:['Nepoznato','Otključan','SIM-lock','Drugi lock']};
const live=r=>(r.instances||[]).filter(u=>ACTIVE.includes(u.condition));
const unique=a=>[...new Set(a.filter(Boolean))];
const name=r=>[r.brand,r.model].filter(Boolean).join(' ');
function toast(msg){const dialog=[...document.querySelectorAll('dialog[open]')].at(-1);let node=$('toast');if(dialog){node=dialog.querySelector('.dialog-notice');if(!node){node=document.createElement('p');node.className='dialog-notice hint';node.setAttribute('role','status');dialog.prepend(node);}}node.textContent=msg;node.hidden=false;clearTimeout(toastTimer);toastTimer=setTimeout(()=>node.hidden=true,9000);if($('update-result')&&$('panel').open)$('update-result').textContent=msg;}
...
function value(r,key){const us=live(r);switch(key){
 ...
 case 'type':return unique([r.type,...us.map(u=>u.type)]).join(', ');
...
function cell(r,key){const v=value(r,key);if(catalogNames[key])return catalogLink(key,v);if(key==='colors')return unique(live(r).map(u=>u.color)).map(v=>catalogLink('color',v)).join(', ');if(key==='state')return esc(unique(live(r).map(u=>enumLabel(u.state))).join(', ')||enumLabel(r.condition)||'—');if(key==='editions')return esc(unique(live(r).map(u=>enumLabel(u.edition))).join(', '));if(key==='image')return image(r);
 if(key==='model')return `<button class="link" data-action="expand" data-id="${r.id}" aria-expanded="${expanded.has(r.id)}">${r.kind==='phone'?(expanded.has(r.id)?'▾ ':'▸ '):''}${esc(r.model)}</button><small>${r.kind==='part'?'Part / accessory':r.wishlist?'On wishlist':''}</small>`;
...
function render(){
 ...
 $('collection-table').innerHTML=`<thead><tr>${visible.map(k=>`<th scope="col" draggable="true" data-column-key="${k}" title="Drag to reorder; click to sort" data-sort="${k}">${esc(columns.find(c=>c[0]===k)[1])}${sort.key===k?(sort.dir===1?' ↑':' ↓'):''}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr>${visible.map(k=>`<td class="${k==='model'?'model-cell':['colors','editions','state','note','location','type'].includes(k)?'wrap':''}">${cell(r,k)}</td>`).join('')}</tr>${expanded.has(r.id)?expandedRow(r,visible.length):''}`).join('')}</tbody>`;
...
function blankUnit(){return {inv:'',color:'',edition:'Standard',state:'Netestiran',condition:'U kolekciji',purpose:'Kolekcija',rating:0,location:'',currency:'CHF',photos:[],box:null,battery_present:null,charger_present:null,manual:null,headphones:null,matching_box:null};}
function newRecord(kind='phone'){return {kind,brand:'',model:'',image:'',instances:[],photos:[],compatible:[],quantity:0,reserved:0,custom:{},specs:{},currency:'CHF',rating:0};}
...
${field('Type code','type',u.type,{index:i})}${field('Product code','product_code',u.product_code,{index:i})}${field('Memory variant','memory',u.memory,{index:i})}${field('Installed firmware / OS','firmware',u.firmware,{index:i})}
...
${field('Purpose','purpose',u.purpose,{index:i,choices:opts.purpose})}${field('Ownership status','condition',u.condition,{index:i,choices:opts.condition})}${field('Originality','originality',u.originality||'Nepoznato',{index:i,choices:opts.originality})}
...
${field('Purchase price','price',u.price||0,{index:i,type:'number'})}${field('Estimated value','value',u.value||0,{index:i,type:'number'})}${field('Currency','currency',u.currency||'CHF',{index:i,choices:opts.currency})}
...
async function settingsPanel(){
 ...
 ${section('about',`<h2>MyPhoneLibrary</h2><p>Version ${esc(db.version)}</p><p class="subtle">Your personal phone, unit and parts collection.</p><div class="actions"><button data-action="update-info">Check and install updates</button></div>`)}
 <div class="actions settings-save"><button data-action="save-settings" class="primary">Save settings</button></div></div></div>`,{kind:'settings'});loadNetwork().catch(e=>{if($('network-status'))$('network-status').textContent=e.message;});
}
...
document.addEventListener('click',async event=>{
 ...
 case 'catalog-delete':{
 const related=[...db.records,...db.trash].filter(r=>[r,...r.instances||[]].some(o=>o.catalog_refs?.[catalogDraft.category]===catalogDraft.id));
 if(!confirm('Delete '+catalogDraft.name+'?'+(related.length?' Used by '+related.length+' records. Their text is kept, but the catalog link and specifications are removed.':'')))break;
 await api('/api/catalog-delete',{id:catalogDraft.id,rev:catalogDraft.rev,confirm_linked:true});await refresh();catalogList(catalogDraft.category);toast('Catalog item deleted.');break;
 }
 ...
 case 'browse-backup':{target.disabled=true;try{const result=await api('/api/backup-folder',{});if(result.path){$(target.dataset.field).value=result.path;panelDirty=true;}}finally{target.disabled=false;}break;}
...
$('collection-table').addEventListener('dragstart',e=>{const th=e.target.closest('[data-column-key]');if(!th)return;dragColumn=th.dataset.columnKey;e.dataTransfer.setData('text/plain',dragColumn);e.dataTransfer.effectAllowed='move';th.classList.add('dragging');});
...
$('collection-table').addEventListener('drop',async e=>{const th=e.target.closest('[data-column-key]');if(!th||!dragColumn)return;e.preventDefault();const keys=[...$('collection-table').querySelectorAll('[data-column-key]')].map(t=>t.dataset.columnKey);const from=dragColumn,to=th.dataset.columnKey;dragColumn=null;ignoreSortUntil=Date.now()+400;if(from===to)return;keys.splice(keys.indexOf(from),1);keys.splice(keys.indexOf(to),0,from);try{await api('/api/settings',{columns:keys});db.settings.columns=keys;render();}catch(error){toast(error.message);}});
