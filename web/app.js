...
function compareUnits(){const entries=db.records.flatMap(r=>(r.instances||[]).filter(u=>selectedUnits.has(u.id)).map(u=>({r,u})));if(entries.length<2||entries.length>4)throw Error('Select 2 to 4 units of the same model to compare.');if(entries.some(e=>e.r.id!==entries[0].r.id))throw Error('Select units of the same model.');const fields=[['Color',u=>u.color],['Edition',u=>enumLabel(u.edition)],['Condition',u=>enumLabel(u.state)],['Ownership',u=>enumLabel(u.condition)],['Cosmetic rating',u=>u.rating],['Location',u=>u.location],['Variant',u=>u.alias||entries[0].r.alias],['Type code',u=>u.type||entries[0].r.type],['Operating system',u=>u.os||entries[0].r.os],...accessoryFields.map(k=>[unitFields[k],u=>u[k]===true?'Yes':u[k]===false?'No':'Not checked']),['Purchased',u=>u.purchase_date],['Source',u=>u.source],['Purchase price',u=>u.price+' '+amountCurrency(u)],['Estimated value',u=>u.value+' '+amountCurrency(u)],['Notes',u=>u.note]];panel('Compare — '+name(entries[0].r),`<p>Differences are highlighted.</p><div class="comparison-scroll"><table class="comparison-table"><thead><tr><th>Property</th>${entries.map(({r,u})=>`<th>${unitImage(r,u)}<strong>#${esc(u.inv)}</strong><button data-action="edit-unit" data-id="${r.id}" data-index="${r.instances.indexOf(u)}">Edit</button></th>`).join('')}</tr></thead><tbody>${fields.map(([label,get])=>{const values=entries.map(({u})=>String(get(u)??''));return `<tr class="${new Set(values).size>1?'comparison-different':''}"><th>${esc(label)}</th>${values.map(v=>`<td>${esc(v)||'—'}</td>`).join('')}</tr>`;}).join('')}</tbody></table></div>`,{kind:'compare'});}
...
function render(){renderFilters();renderWishlistOverview();
...
document.addEventListener('click',async event=>{
 ...
 const leavesPanel=['saved-filters','locations','compare-units','unit-details','location-contents','bulk-edit','nav-view','settings','catalogs','catalog-list','catalog-item','catalog-new','add-phone','add-part','trash','backups','inventory','columns','custom-fields','password','update-info','import','export'];
