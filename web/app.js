...
function controlValue(key,el){return accessoryFields.includes(key)?el.value==='true'?true:el.value==='false'?false:null:el.value;}
function startQuick(target){if(quickEdit)throw Error('Save or cancel the current cell first.');const r=record(target.dataset.id),u=r.instances[Number(target.dataset.index)],key=target.dataset.field;
 quickEdit={record_id:r.id,unit_id:u.id,rev:r.rev,key};const td=target.closest('td');td.innerHTML=`<div class="quick-control">${unitControl(key,u[key],'quick-value')}<button data-action="quick-save">Save</button><button data-action="quick-cancel">Cancel</button></div>`;$('quick-value').focus();}
...
<p class="span-all subtle">Variant, type code, operating system and links below apply to this unit. Leave blank to inherit the model's value.</p>
${field('Model number / Variant','alias',u.alias,{index:i})}${field('Type code','type',u.type,{index:i})}${field('Operating system','os',u.os,{index:i,list:'options-os'})}${field('GSMArena link','gsm',u.gsm,{index:i,type:'url'})}${field('Wikipedia link','wiki',u.wiki,{index:i,type:'url'})}
...
<div class="completeness-summary">${completenessHTML(u)}</div><small>These details belong to this physical unit only. IMEI is optional.</small></div></details>`;}
...
${part?rf('Catalog item','catalog_item',{choices:[['','None'],...db.catalog.filter(x=>['battery','charger','part_category'].includes(x.category)).map(x=>[x.id,x.name])] }):''}
...
function backupContents(){return `<h3>Backups and recovery</h3><p class="subtle">Data folder: ${esc(db.data_directory||'')}</p><p>The ZIP contains your database, photos and settings. Current data is backed up before restoring.</p><div class="actions"><button class="primary" data-action="create-backup">Create backup</button><label>Restore from ZIP backup<input id="restore-file" type="file" accept=".zip"></label></div><div id="backup-files" class="settings-list"></div>`;}
async function loadBackups(){const files=await api('/api/backups');if($('backup-files'))$('backup-files').innerHTML=files.map(f=>`<div class="setting-row"><span>${esc(f.name)} <small>${(f.bytes/1024/1024).toFixed(2)} MB</small></span><a href="/api/download-backup?name=${encodeURIComponent(f.name)}" download>Download</a></div>`).join('')||'<p class="subtle">No backups yet.</p>';}
...
document.addEventListener('click',async event=>{
 ...
 try{
 if(quickEdit&&!['quick-start','quick-save','quick-cancel'].includes(action)){if(!confirm('Discard the unsaved cell change?'))return;quickEdit=null;render();}
 ...
 case 'catalog-save':{const specs={};for(const row of document.querySelectorAll('.spec-row')){const key=row.querySelector('[data-spec-key]').value.trim(),value=row.querySelector('[data-spec-value]').value;if(!key&&value)throw Error('Enter a property name.');if(key){if(key in specs)throw Error('Property names must be unique.');specs[key]=value;}}const saved=await api('/api/catalog',{...catalogDraft,name:$('catalog-name').value,description:$('catalog-description').value,source:$('catalog-source').value,parent_id:$('location-parent')?.value||'',compatible:[...document.querySelectorAll('[data-catalog-compatible]:checked')].map(x=>x.dataset.catalogCompatible),specs});await refresh();catalogEditor(saved.category,saved.id);toast('Catalog item saved. Linked records updated.');break;}
...
$('record-form').addEventListener('submit',async e=>{e.preventDefault();if(uploadCount){toast('Photos are still uploading.');return;}readDraft();const duplicates=imeiWarnings()||[];if(duplicates.length&&!confirm('Duplicate IMEI detected. Review the warning above. Save anyway?'))return;$('record-save').disabled=true;try{const r=await api('/api/record',{record:draft,rev:draft.rev});await refresh();if(mode==='add')phoneDraft=null;dirty=false;$('editor').close();draft=null;expanded.add(r.id);render();toast('Phone / record saved.');}catch(error){$('editor-error').textContent=error.message;}finally{$('record-save').disabled=false;}});
