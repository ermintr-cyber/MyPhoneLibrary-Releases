...
function startQuick(target){if(quickEdit)throw Error('Save or cancel the current cell first.');const r=record(target.dataset.id),u=r.instances[Number(target.dataset.index)],key=target.dataset.field;
 quickEdit={record_id:r.id,unit_id:u.id,rev:r.rev,key};const td=target.closest('td');td.innerHTML=`<div class="quick-control">${unitControl(key,u[key],'quick-value')}<button data-action="quick-save">Save</button><button data-action="quick-cancel">Cancel</button></div>`;($('quick-value').closest?.('.catalog-combo')?.querySelector('[data-combo-query]')||$('quick-value')).focus();}
async function saveQuick(){if(!quickEdit)return;const q=quickEdit;await api('/api/bulk-units',{targets:[q],changes:{[q.key]:controlValue(q.key,$('quick-value'))}});quickEdit=null;await refresh();toast('Unit updated.');}
...
async function unitDetails(id,index){const r=record(id),u=r?.instances[index];if(!u)return;const history=await api('/api/history?id='+encodeURIComponent(id));const effective=k=>u[k]||r[k]||'—';
 const pairs=[['Brand',r.brand],['Model',r.model],['Inventory number',u.inv],['Model number / Variant',effective('alias')],['Type code',effective('type')],['Product code',u.product_code],['Color',u.color],['Edition',enumLabel(u.edition)],['Operating system',effective('os')],['Firmware',u.firmware],['Battery',r.battery],['Charger',r.charger],['IMEI',u.imei],['IMEI 2',u.imei2],['Serial number',u.serial],['Condition',enumLabel(u.state)],['Location',u.location],['Purchased',u.purchase_date],['Source',u.source],['Purchase price',u.price+' '+amountCurrency(u)],['Estimated value',u.value+' '+amountCurrency(u)]];
...
const filterLabels={brand:'Brand',color:'Color',os:'Operating system',location:'Location',state:'Condition'};
function renderFilters(){const root=$('quick-filters');if(!root)return;root.innerHTML=Object.entries(filterLabels).map(([k,label])=>{const values=unique(db.records.flatMap(r=>k==='brand'?[r.brand]:r.kind==='part'?[k==='state'?r.condition:r[k]]:(r.instances||[]).map(u=>u[k]||(k==='os'?r.os:'')))).sort((a,b)=>a.localeCompare(b));if(quickFilters[k]&&!values.includes(quickFilters[k]))values.push(quickFilters[k]);return `<label>${label}<select data-quick-filter="${k}"><option value="">All</option>${values.map(v=>`<option value="${esc(v)}" ${v===quickFilters[k]?'selected':''}>${esc(enumLabel(v))}</option>`).join('')}</select></label>`;}).join('')+'<button data-action="clear-filters">Clear filters</button>';}
function matchesFilters(r,u){return Object.entries(quickFilters).every(([k,v])=>!v||normal(k==='brand'?r.brand:k==='os'?(u?.os||r.os):u?u[k]:k==='state'?r.condition:r[k])===normal(v));}
...
function filtered(){
 ...
 let rows=db.records.filter(r=>{if(view==='incomplete'&&!live(r).some(u=>completeness(u).missing.length))return false;if(view==='phone'||view==='part'){if(r.kind!==view)return false;}if(view==='wish'&&!r.wishlist&&!(r.instances||[]).some(u=>u.condition==='Wanted'))return false;if(view==='duplicates'&&live(r).length<2)return false;if(view==='untested'&&!live(r).some(u=>u.state==='Netestiran'))return false;if(view==='repair'&&!live(r).some(u=>u.purpose==='Za popravak'||u.state==='Neispravan'||u.state==='Djelimično ispravan'))return false;return searchMatches(r,q);});
...
function cell(r,key){if(key==='select')return selectionBox(r);if(key==='completeness')return r.kind==='phone'?unique(live(r).map(u=>completeness(u).missing.length?'Incomplete':completeness(u).unknown.length?'Not checked':'Complete')).join(', '):'—';const v=value(r,key);if(key==='os'&&r.kind==='phone')return unique(live(r).map(u=>u.os||r.os).filter(Boolean)).map(x=>catalogLink('os',x)).join(', ')||catalogLink('os',r.os);if(catalogNames[key])return catalogLink(key,v);if(key==='colors')return unique(live(r).map(u=>u.color)).map(v=>catalogLink('color',v)).join(', ');if(key==='state')return esc(unique(live(r).map(u=>enumLabel(u.state))).join(', ')||enumLabel(r.condition)||'—');if(key==='editions')return esc(unique(live(r).map(u=>enumLabel(u.edition))).join(', '));if(key==='image')return `<div class="model-image-cell">${r.kind==='phone'?`<button class="expand-model" data-action="expand" data-id="${r.id}" aria-label="Show or hide units" aria-expanded="${expanded.has(r.id)}">${expanded.has(r.id)?'▾':'▸'}</button>`:'<span class="expand-spacer"></span>'}${image(r)}</div>`;
...
function unitCell(r,u,i,key){
 ...
 if(quickKey)return quickButton(r,u,i,quickKey,key==='box'?triLabel(u.box):esc(enumLabel(u[quickKey]||(key==='os'?r.os:''))));
 ...
 if(key==='note')return u.note?`<span title="${esc(u.note)}">${esc(u.note)}</span>`:'—';
...
function catalogCombo(category,value,attr='',required=false){
 ...
 return `<div class="catalog-combo" data-category="${esc(category)}" data-original="${esc(value||'')}"><input type="hidden" ${attr} value="${esc(value||'')}"><div class="combo-input"><input data-combo-query role="combobox" aria-label="${esc(catalogNames[category])}" aria-autocomplete="list" aria-controls="${id}" aria-expanded="false" autocomplete="off" value="${esc(value||'')}" ${required?'required':''}><button type="button" data-action="combo-toggle" aria-label="Show ${esc(catalogNames[category])}">▾</button></div><div id="${id}" class="combo-menu" role="listbox" hidden></div></div>`;
}
...
function editorRender(unitIndex=null){
 ...
 ${part?rf('Part category','part_category',{list:'options-part_category'}):''}${rf('Battery / code','battery',{list:'options-battery'})}${rf('Charger / connector','charger',{list:'options-charger'})}
 ...
 ${rf('Unverified quantity','declared_qty')}${rf('Unverified legacy parts count','declared_parts')}
...
document.addEventListener('click',async event=>{
 ...
 const leavesPanel=['unit-details','location-contents','bulk-edit','nav-view','settings','catalogs','catalog-list','catalog-item','catalog-new','add-phone','add-part','trash','backups','inventory','columns','custom-fields','password','update-info','import','export'];
...
$('login-form').addEventListener('submit',async e=>{e.preventDefault();$('login-error').textContent='';$('login-submit').disabled=true;try{await api($('login-form').dataset.setup==='true'?'/api/setup':'/api/login',{password:$('password').value,remember:$('remember-me').checked});$('password').value='';await boot();}catch(error){$('login-error').textContent=error.message;}finally{$('login-submit').disabled=false;}});
