...
function editorRender(unitIndex=null){
 ...
 ${part?rf('Part category','part_category',{list:'options-part_category'}):''}${rf('Battery / code','battery',{list:'options-battery'})}${rf('Charger / connector','charger',{list:'options-charger'})}<div id="editor-battery-alternatives" class="span-all subtle">${batteryAlternatives(draft.battery).length?'Compatible batteries: '+batteryAlternatives(draft.battery).map(x=>esc(x.name)).join(' · '):''}</div>
