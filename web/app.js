...
async function unitDetails(id,index){const r=record(id),u=r?.instances[index];if(!u)return;const history=await api('/api/history?id='+encodeURIComponent(id));const effective=k=>u[k]||r[k]||'—';
 const pairs=[['Brand',r.brand],['Model',r.model],['Inventory number',u.inv],['Model number / Variant',effective('alias')],['Type code',effective('type')],['Product code',u.product_code],['Color',u.color],['Edition',enumLabel(u.edition)],['Operating system',effective('os')],['Firmware',u.firmware],['Memory',u.memory],['Battery',[r.battery,...batteryAlternatives(r.battery).map(x=>x.name+' (compatible)')].filter(Boolean).join(' · ')],['Charger',r.charger],['IMEI',u.imei],['IMEI 2',u.imei2],['Serial number',u.serial],['Condition',enumLabel(u.state)],['Location',u.location],['Purchased',displayPurchaseDate(u.purchase_date)],['Source',u.source],['Purchase price',u.price+' '+amountCurrency(u)],['Estimated value',u.value+' '+amountCurrency(u)]];
...
function modelValues(r,key){
 if(r.kind!=='phone')return null;
 const inherited=['alias','type','os','gsm','wiki'];
 const direct={colors:'color',editions:'edition',state:'state',location:'location',product_code:'product_code'};
 if(!inherited.includes(key)&&!direct[key]&&!['rating','box','value','completeness'].includes(key))return null;
 const units=live(r),values=units.map(u=>{
  if(inherited.includes(key))return u[key]||r[key]||'';
  if(direct[key])return enumLabel(u[direct[key]]||'');
  if(key==='rating')return u.rating?'★ '+u.rating:'';
  if(key==='box')return u.box===true?'Yes':u.box===false?'No':'Not checked';
  if(key==='value')return Number(u.value)?u.value+' '+(u.currency||r.currency||'KM'):'';
  const c=completeness(u);return c.missing.length?'Incomplete':c.unknown.length?'Not checked':'Complete';
 });
 if(!units.length&&inherited.includes(key))values.push(r[key]||'');
 const seen=new Set();return values.map(v=>String(v).trim()).filter(v=>{const identity=['gsm','wiki'].includes(key)?v:normal(v);if(!v||seen.has(identity))return false;seen.add(identity);return true;});
}
function modelSummary(r,key){const values=modelValues(r,key);if(values===null)return null;if(!values.length)return '—';
 if(values.length>1){const label=columns.find(c=>c[0]===key)?.[1]||key;return `<button type="button" class="note-icon multiple-value" data-action="note-popup" data-note="${esc(label+':\n'+values.join('\n'))}" aria-label="${esc(label)}: Multiple — show values">Multiple</button>`;}
 const v=values[0],category={colors:'color',location:'location',os:'os'}[key];if(category)return catalogLink(category,v);
 if(key==='gsm'||key==='wiki')return `<a href="${esc(v)}" target="_blank" rel="noopener noreferrer">${key==='gsm'?'GSM':'Wiki'}</a>`;
 return esc(v);
}
function cell(r,key){return checkCell(r,null,key,modelSummary(r,key)??rawCell(r,key));}
...
function phoneCard(r){return `<article class="phone-card"><div class="card-image">${image(r)}<span class="card-qty">${esc(value(r,'qty'))} ${r.kind==='phone'?'units':'parts'}</span></div><div class="card-content"><small>${catalogLink('brand',r.brand)}</small><h2>${esc(r.model)}</h2>${checkSummary(r)}${isWanted(r)?'<small class="wanted-tag">Wanted</small>':''}${acquireButton(r)}<p>${r.kind==='phone'?cell(r,'colors'):esc(r.part_category||'—')}</p><p class="subtle">${r.kind==='phone'?cell(r,'editions'):''}</p><dl><div><dt>Battery</dt><dd>${batteryCell(r.battery)}</dd></div><div><dt>Charger</dt><dd>${catalogLink('charger',r.charger)}</dd></div></dl><div class="actions">${actionButton(r)}${r.kind==='phone'?`<button class="primary" data-action="card-units" data-id="${r.id}">View units</button>`:`<button data-action="move" data-id="${r.id}">Stock</button>`}</div></div></article>`;}
...
function modelSpecifications(r){return `<div class="model-specifications">${[['Colors','colors'],['Editions','editions'],['Mod. nr.','alias'],['Type','type'],['Battery','battery'],['Charger','charger'],['OS','os']].filter(([,key])=>modelValues(r,key)?.length||r[key]).map(([label,key])=>`<span><small>${label}</small>${cell(r,key)}</span>`).join('')}</div>`;}
...
function comparisonText(value){if(value===null||value===undefined)return '';if(typeof value==='boolean')return value?'Yes':'No';if(Array.isArray(value))return value.map(comparisonText).filter(Boolean).join(' · ');if(typeof value==='object')return Object.entries(value).map(([k,v])=>comparisonText(v)?k+': '+comparisonText(v):'').filter(Boolean).join(' · ');return String(value).trim();}
function comparisonRows(entries,includeEmpty=false){
 const unit=k=>(u)=>u[k],model=k=>(u,r)=>r[k],effective=k=>(u,r)=>u[k]||r[k];
 const money=k=>(u)=>u[k]===null||u[k]===undefined||u[k]===''?'':u[k]+' '+amountCurrency(u);
 const fields=[['Brand',model('brand')],['Model',model('model')],['Inventory number',unit('inv')],['Variant',effective('alias')],['Type code',effective('type')],['Product code',unit('product_code')],['Color',unit('color')],['Edition',u=>enumLabel(u.edition)],['Memory',unit('memory')],['Operating system',effective('os')],['Firmware',unit('firmware')],['Battery',model('battery')],['Charger',model('charger')],['Released',model('released')],['Introduced',model('introduced')],['IMEI',unit('imei')],['IMEI 2',unit('imei2')],['Serial number',unit('serial')],['Condition',u=>enumLabel(u.state)],['Ownership',u=>enumLabel(u.condition)],['Purpose',u=>enumLabel(u.purpose)],['Donor phone',u=>u.for_parts==null?(u.purpose==='Donor'?true:null):u.for_parts],['Cosmetic rating',unit('rating')],['Location',unit('location')],['Lock status',u=>enumLabel(u.lock)],...accessoryFields.map(k=>[k==='battery_present'?'Battery included':k==='charger_present'?'Charger included':unitFields[k],unit(k)]),['Matching box IMEI',unit('matching_box')],['Purchased',u=>displayPurchaseDate(u.purchase_date)],['Source',unit('source')],['Purchase price',money('price')],['Estimated value',money('value')],['Currency',u=>amountCurrency(u)],['Notes',unit('note')],['Model notes',model('note')],['GSMArena',effective('gsm')],['Wikipedia',effective('wiki')],['Favorite',model('favorite')],['Wishlist',model('wishlist')],['Wishlist priority',model('wish_priority')],['Target price',(u,r)=>r.wish_price==null||r.wish_price===''?'':r.wish_price+' '+amountCurrency(r)],['Wishlist notes',model('wish_note')],['To check',(u,r)=>[...new Set([...checkFields(r),...checkFields(u)])].map(checkLabel).join(' · ')]];
 const customKeys=[...new Set([...(db.settings.custom_fields||[]).map(f=>f.id),...entries.flatMap(({r})=>Object.keys(r.custom||{}))])];
 for(const key of customKeys){const f=(db.settings.custom_fields||[]).find(f=>f.id===key);fields.push(['Custom: '+(f?.label||key),(u,r)=>r.custom?.[key],'custom:'+key]);}
 const specKeys=[...new Set(entries.flatMap(({r})=>Object.keys(r.specs||{})))];
 for(const key of specKeys)fields.push(['Specification: '+key,(u,r)=>r.specs?.[key],'spec:'+key]);
 return fields.map(([label,get,id])=>({id:id||label,label,values:entries.map(({u,r})=>comparisonText(get(u,r)))})).filter(row=>includeEmpty||row.values.some(v=>v!==''));
}
function compareUnits(fieldsOpen=false){const entries=db.records.flatMap(r=>(r.instances||[]).filter(u=>selectedUnits.has(u.id)).map(u=>({r,u})));if(entries.length<2||entries.length>20)throw Error('Select 2 to 20 phones to compare. Different models are welcome.');const all=comparisonRows(entries,true),hidden=new Set(db.settings.comparison_hidden||[]);panel('Compare — '+entries.length+' phones',`<p>Differences are highlighted. Fields empty on every selected phone are hidden.</p><details class="comparison-fields" ${fieldsOpen?'open':''}><summary>Fields</summary><p class="subtle">Your choices are saved for this library, including after restarting the app.</p><div class="comparison-field-options">${all.map(({id,label,values})=>`<label class="check"><input type="checkbox" data-compare-field="${esc(id)}" ${hidden.has(id)?'':'checked'}>${esc(label)}${values.every(v=>v==='')?' <small>No data</small>':''}</label>`).join('')}</div></details><div class="comparison-scroll"><table class="comparison-table"><thead><tr><th>Property</th>${entries.map(({r,u})=>`<th>${unitImage(r,u)}<strong>${esc(name(r))}</strong><strong>#${esc(u.inv)}</strong><button data-action="edit-unit" data-id="${r.id}" data-index="${r.instances.indexOf(u)}">Edit</button></th>`).join('')}</tr></thead><tbody>${all.filter(({id,values})=>!hidden.has(id)&&values.some(v=>v!=='')).map(({label,values})=>`<tr class="${new Set(values).size>1?'comparison-different':''}"><th>${esc(label)}</th>${values.map(v=>`<td>${esc(v)||'—'}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`,{kind:'compare'});}
...
function displayPurchaseDate(value){const text=String(value||'').trim(),m=text.match(/^([0-9]{4})-([0-9]{2})-([0-9]{2})$/);return m?m[3]+'.'+m[2]+'.'+m[1]:text;}
function parsePurchaseDate(value){const text=String(value||'').trim();if(!text)return '';const m=text.match(/^([0-9]{2})[.]([0-9]{2})[.]([0-9]{4})$/);if(!m)return null;const day=Number(m[1]),month=Number(m[2]),year=Number(m[3]),date=new Date(0);date.setUTCFullYear(year,month-1,day);if(year<1||date.getUTCFullYear()!==year||date.getUTCMonth()!==month-1||date.getUTCDate()!==day)return null;return m[3]+'-'+m[2]+'-'+m[1];}
function validatePurchaseDates(){for(const el of $('editor-content').querySelectorAll('[data-date-dmy]')){el.setCustomValidity(parsePurchaseDate(el.value)===null?'Enter a valid date in dd.mm.yyyy format, e.g. 25.09.2026.':'');if(!el.reportValidity())return false;}return true;}
document.addEventListener('input',e=>{if(e.target.matches?.('[data-date-dmy]'))e.target.setCustomValidity(parsePurchaseDate(e.target.value)===null?'Enter a valid date in dd.mm.yyyy format, e.g. 25.09.2026.':'');});
function field(label,key,val,{index=null,type='text',list='',choices=null,required=false}={}){
 ...
 let control;
 if(key==='purchase_date')control=`<input ${attr} data-date-dmy type="text" placeholder="dd.mm.yyyy" value="${esc(displayPurchaseDate(val))}" pattern="[0-9]{2}[.][0-9]{2}[.][0-9]{4}" maxlength="10" title="dd.mm.yyyy — e.g. 25.09.2026" autocomplete="off">`;
 else if(choices){const a=choices.map(v=>Array.isArray(v)?v:[v,v]);if(val!==undefined&&val!==null&&val!==''&&!a.some(([v])=>String(v)===String(val)))a.unshift([val,val]);control=`<select ${attr}>${a.map(([v,l])=>`<option value="${esc(v)}" ${String(v)===String(val??'')?'selected':''}>${esc(enumLabel(l))}</option>`).join('')}</select>`;}
...
${field('Acquisition date (dd.mm.yyyy)','purchase_date',u.purchase_date,{index:i,type:'date'})}${field('Purchased from / source','source',u.source,{index:i})}${field('Lock status','lock',u.lock||'Nepoznato',{index:i,choices:opts.lock})}
...
document.addEventListener('change',async e=>{const el=e.target;try{
 ...
 if(el.dataset.compareField){const hidden=new Set(db.settings.comparison_hidden||[]);if(el.checked)hidden.delete(el.dataset.compareField);else hidden.add(el.dataset.compareField);const controls=[...document.querySelectorAll('[data-compare-field]')];controls.forEach(c=>c.disabled=true);try{const saved=await api('/api/settings',{comparison_hidden:[...hidden]});db.settings.comparison_hidden=saved.comparison_hidden;compareUnits(true);}catch(error){el.checked=!el.checked;throw error;}finally{controls.forEach(c=>c.disabled=false);}}
