...
await page.evaluate(()=>{const rows=db.records.filter(r=>r.instances.some(u=>selectedUnits.has(u.id)));rows[0].instances[0].memory='8 GB';rows[0].instances[0].firmware='V 30';rows[0].instances[0].matching_box=false;rows[0].specs={...rows[0].specs,Camera:'5 MP',Empty:' '};compareUnits();});
await page.locator('.comparison-fields summary').click();
await page.locator('[data-compare-field="Memory"]').uncheck();
await page.waitForFunction(()=>db.settings.comparison_hidden?.includes('Memory'));
assert.equal(await page.locator('.comparison-table tbody tr').filter({has:page.locator('th',{hasText:/^Memory$/})}).count(),0);
await page.evaluate(()=>{compareUnits(true);});
assert.equal(await page.locator('[data-compare-field="Memory"]').isChecked(),false);
await page.locator('[data-compare-field="Memory"]').check();
await page.waitForFunction(()=>!db.settings.comparison_hidden?.includes('Memory'));
await page.locator('.comparison-fields summary').click();
const comparisonRowsDOM=page.locator('.comparison-table tbody tr');
assert.equal(await comparisonRowsDOM.filter({has:page.locator('th',{hasText:/^Memory$/})}).count(),1);
assert.match(await page.locator('.comparison-table').textContent(),/8 GB/);
assert.match(await page.locator('.comparison-table').textContent(),/Specification: Camera/);
assert.ok(!(await page.locator('.comparison-table').textContent()).includes('Specification: Empty'));
