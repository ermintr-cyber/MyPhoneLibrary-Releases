...
vm.runInContext("currentView='incomplete';render()",context);assert.doesNotMatch(node('completion-report').innerHTML,/Nokia N73/);vm.runInContext("db.records[0].instances[0].box=true;db.records[0].instances[0].charger_present=false;render()",context);assert.match(node('completion-report').innerHTML,/Nokia N73/);assert.match(node('completion-report').innerHTML,/Missing: Charger/);assert.doesNotMatch(node('completion-report').innerHTML,/Spare battery/);assert.equal(vm.runInContext("needsCompletion({box:true,purpose:'Donor'})",context),false);vm.runInContext("currentView='all'",context);
vm.runInContext("catalogEditor('location','box')",context);assert.match(node('panel-body').innerHTML,/location-parent/);
vm.runInContext("catalogEditor('battery','bat')",context);assert.match(node('panel-body').innerHTML,/data-battery-spec="Voltage"/);assert.ok(!node('panel-body').innerHTML.includes('Additional compatible'));assert.ok(node('panel-body').innerHTML.includes('catalog-source'));assert.ok(node('panel-body').innerHTML.includes('catalog-description'));
